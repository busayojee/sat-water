...
class Inference:
    ...
    def predict(self,image, mask="", image2 = None, fname=None):
        ...
        if isinstance(self.model, dict):
            images= (image - tf.reduce_min(image)) / (tf.reduce_max(image) - tf.reduce_min(image))
            ...
            if image2!=None:
                images2= (image2 - tf.reduce_min(image2)) / (tf.reduce_max(image2) - tf.reduce_min(image2))
                ...
                Preprocess.label_overlay(images2, prediction3, fname=f"segmentation/segment-water/results/{fname}.png")  
                prediction3 = {"Resnet34(512,512)":prediction3}
                Inference.plot_image(images, prediction1, prediction2, prediction3, fname=fname)
                     
            else: 
                Inference.plot_image(images, mask, prediction1, prediction2, fname="multiple")
        else:
            if self.name == "unet":
