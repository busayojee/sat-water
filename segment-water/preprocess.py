...
class Preprocess:
    ...
    def resize_test(data,size):
        ...
        return image
    @staticmethod
    def data_load(dataset, mask_folder, image_folder,split, shape, batch_size,channels= 0):
        dataset = [dataset]
        ds = []
        for f in dataset:
        ...
        train_ds = train_ds.map(lambda x: Preprocess.load( x, mask_folder, image_folder, channels))
        train_ds = train_ds.map(Preprocess.make_single_class)
        train_ds = train_ds.map(lambda x: Preprocess.resize(x, shape), num_parallel_calls=AUTOTUNE)
        # train_ds = train_ds.map(make_single_class)
        train_ds = train_ds.batch(batch_size)
        train_ds = train_ds.prefetch(buffer_size=AUTOTUNE)
        val_ds = val_ds.map(lambda x: Preprocess.load( x, mask_folder, image_folder, channels))
        val_ds = val_ds.map(Preprocess.make_single_class)
        val_ds = val_ds.map(lambda x: Preprocess.resize(x, shape), num_parallel_calls=AUTOTUNE)
      
        val_ds = val_ds.batch(batch_size)
        val_ds = val_ds.prefetch(buffer_size=AUTOTUNE)
        test_ds = test_ds.map(lambda x: Preprocess.load( x, mask_folder, image_folder, channels))
        test_ds = test_ds.map(Preprocess.make_single_class)
        test_ds = test_ds.map(lambda x: Preprocess.resize(x, shape), num_parallel_calls=AUTOTUNE)
    ...
    def plot_image(train_ds, n=1, fname=None):
       for image, mask in train_ds.unbatch().shuffle(buffer_size= len(train_ds)).take(1):
